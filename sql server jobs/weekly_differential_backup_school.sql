...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'weekly_differential_backup_school', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Weekly backup of the SchoolDB database every saturday at 4:00 AM', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'backup scheduling', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=64, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'f0c848e0-f7db-4dab-b5da-39839c5c3639'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
